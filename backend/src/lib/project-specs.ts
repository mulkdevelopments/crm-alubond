...
export function commercialSpecsComplete(
  thickness: string,
  core: string,
  paintType: string,
): boolean {
...
export function resolveItemNameFromSpecs(
  thickness: string,
  core: string,
  paintType: string,
  fallbackItemName = "",
) {
  if (commercialSpecsComplete(thickness, core, paintType)) {
    return formatProjectSpecs(thickness, core, paintType);
  }
